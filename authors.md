# Authors and Citation

## Authors

- **Martijn Tennekes**. Author, maintainer.

- **Jakub Nowosad**. Contributor.

- **Joel Gombin**. Contributor.

- **Sebastian Jeworutzki**. Contributor.

- **Kent Russell**. Contributor.

- **Richard Zijdeman**. Contributor.

- **John Clouse**. Contributor.

- **Robin Lovelace**. Contributor.

- **Jannes Muenchow**. Contributor.

- **Olivier Roy**. Contributor.

- **Edzer Pebesma**. Contributor.

- **Hugh Graham**. Contributor.

- **Michael D. Sumner**. Contributor.

- **Tim Appelhans**. Contributor.

- **Nick Bearman**. Contributor.

- **Pukar Bhandari**. Contributor.

- **Stéphane Guillou**. Contributor.

- **Monika Anna Tomaszewska**. Contributor.

- **Ajoke Onojeghuo**. Contributor.

- **Jerome Guelat**. Contributor.

- **Artur Wroblewski**. Contributor.

## Citation

Source:
[`inst/CITATION`](https://github.com/r-tmap/tmap/blob/4.4/inst/CITATION)

Tennekes M (2018). “tmap: Thematic Maps in R.” *Journal of Statistical
Software*, **84**(6), 1–39.
[doi:10.18637/jss.v084.i06](https://doi.org/10.18637/jss.v084.i06).

    @Article{,
      title = {{tmap}: Thematic Maps in {R}},
      author = {Martijn Tennekes},
      journal = {Journal of Statistical Software},
      year = {2018},
      volume = {84},
      number = {6},
      pages = {1--39},
      doi = {10.18637/jss.v084.i06},
    }
