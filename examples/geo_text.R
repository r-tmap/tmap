...
geo_shape(cities) +
    geo_text("name", cex="pop_max", scale=1, ymod=-.02, root=4, cex.lowerbound = .60, 
             bg.color="yellow", bg.alpha = 150) + 
    geo_bubbles("pop_max", "red", border.col = "black", border.lwd=1, size.lim = c(0, 2e7)) +
geo_shape(Europe) +
    geo_text("name", cex="area", scale=1.5, root=8, cex.lowerbound = .40, 
             fontface="bold", case=NA, fontcolor = "gray35") + 
geo_theme_Europe("Map of Europe", 
