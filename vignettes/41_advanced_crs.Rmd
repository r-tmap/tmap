---
title: "tmap advanced: map projections (CRS)"
output: 
...
tm_shape(World, 
	...
	tm_xlab("Longitudes", size = 1.3) +
	tm_ylab("Latitudes", size = 1.3)
```
...
Besides using latitude longitude coordinates as refences for real locations on earth, they can obviously also be used to draw maps:
...
World_to_polygons = sf::st_cast(World, "POLYGON")
World_to_polygons$area = sf::st_area(World_to_polygons)

tm_shape(World_to_polygons) +
tm_cartogram_ncont(size = "area", options = opt_tm_cartogram_ncont(expansion = 0.4)) +
...
Latitudes longitudes can also be seen as a CRS, but *unprojected*.

In `tmap` and most other spatial data packages in R (`sf`, `stars`, `terra`) the CRS is named in many functions and function arguments `crs` (so small case).

The WGS84 latitude longitude coordinate system can be obtained with EPSG (a library of standards CRSs) number 4326. The formal definition of this CRS can be obtained via `sf::st_crs(4326)`.

For world maps there are several map projections that are useful: plotted areas are (almost) proportional to the real areas, while the polygon shapes are not distorted too much. These projections, belong to the family of "pseudo-cylindrical" projections (see https://r-tmap.github.io/tmap-book/geodata.html#crs
...
do.call(tmap_arrange, c(tms, list(ncol= 3)))
```
