...
Each map variable, e.g. `fill` in `tm_polygons` will by default produce a legend. The type of legend depends on used scale (see \href{https://r-tmap.github.io/tmap/articles/basics_scales}{vignette about scales}). E.g. `tm_scale_continuous` applied to `fill` will produce a continuous color gradient:
...
Each map variable also has a `.legend` argument, such as `fill.legend` in `tm_polygons`. This can be used to:
