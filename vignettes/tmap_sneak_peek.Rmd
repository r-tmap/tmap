...
* Map layers: we are not limited anymore by the fixed set of `tm_polygons`, `tm_lines`, `tm_symbols`, and `tm_raster` (and their derivatives), but any layer of interested can be developed as extension. As an example, we will illustrate `tm_cartogram`. Other layers that would be worthwhile are `tm_network`, `tm_donuts`, `tm_hillshade`, etc.

* Aesthetics: any visual variable can be included in a map layer. We will illustrate this in the next section, where we already implemented 5 new aesthetics for `tm_polygons`.
...
* Spatial data classes: **tmap v3** is build upon `sf` and `stars`. This will also be the case for **tmap v4**, but for developers, it will be easier to incorporate other classes as well, like `SpatRaster` and `SpatVector` from `terra`.


## Aesthetics

The number of aesthetics (by which we mean visual variables) will be much greater, as will be illustrated below. 
...
## New map layers: cartogram example

It will be easy for developers to add new map layers as extension. This will be illustrated by a new map layer, `tm_cartogram`.
