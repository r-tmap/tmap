...
# discrete skill is not possible directly, but only  via interval breaks:
tm_shape(Africa) +
...
An example is the bivaraite choropleth, which is not yet implemented, but will definitly be.
...
However, the framework used in **tmapp v4** also facilitates developers to write plotting methods for other modes.
