...
This vignettes builds on the introduction vignettes about [legends](https://r-tmap.github.io/tmap/articles/basics_legends) and [scales](https://r-tmap.github.io/tmap/articles/basics_scales)
...
When there are multiple data-driven map variables for which legends are produced, there are a couple of things to consider
...
This method, where map variables are passed on via `tm_legend` will be covered in the section about [setting visual values](https://r-tmap.github.io/tmap/articles/adv_legends#setting-visual-values).


### Combining legends

It is also possible to combine legends, which is useful if two map variables are used to plot the same data variable:
...
It is possible to create a custom legend by specifying the visual values that define the legend items. There are two ways to do this: either to change the visual values of an existing legend via the `.legend` argument, or via a new manual legend added with `tm_add_legend()`.
