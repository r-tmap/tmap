...
All options with the name prefix `value(s)` refer to default values for map variables/values. E.g. `value.const` and subitem `fill.polygons` is the default polygon fill color.

The `scales.var` options specify which scales are used by default to map data variables to map variables. This depends on the map variable and the data type. E.g. for numeric data ("num") and the map variable `size`, the `continuous` scale is used, so `tm_scale_continuous()`. For the map variable `text` (of `tm_text()`) the scale `asis` is used, so `tm_scale_asis()`.
