...
The **tmap** package is used to create static and interactive maps with just a few lines of code. It is based on a framework that is similar to the Grammar of Graphics, on which **ggplot2** is build. 
...
* There will be two types of data variables to feed the map layers: `transformation variables` and `visual variables`. A transformation variable is used to change the spatial coordinates (for instance, a cartogram). A visual variable only changes the appearance of a spatial object, e.g. fill color or line width. See below.
