...
Recently, the `sf` package has replaced `sp` as the leading package for processing for vectorized geospatial data. Although tmap 1.x already supported `sf` objects, it was built upon `sp`.

tmap version 2.0 is fully built on `sf`. For raster objects, it still uses `raster`.

An `sf` object (which stands for simple features) has a less complex structure than `sp` objects. An `sf` object is nothing more than a `data.frame` with a special geometry column, which contains the geometries for the corresponding rows. Such a geometry can be a spatial point(s), line(s), polygon(s), or any combination. The layers functions, such as `tm_polygons`, will only draw what they are supposed to draw (in this case polygons). The newly added layer function `tm_sf` will draw all geometries.

Finally, all data objects in `tmap` (except `land`) have been transformed into `sf` objects. The object `land` has been transformed into a `RasterBrick`.
...
In version 2.0, the options are stored in a similar fashion to the global options. Which the function `tmap_options` a list can be retrieved with tmap options, or options can be set. Its behavior is similar to the function `options`.
...
The argument filter has been added to `tm_shape`. Features that are included will be visualized as usual. Excluded features will still be visualized but without mapped color. Instead, they will be colored with `colorNULL`, and a new argument of the layer functions. Note the difference with `colorNA`, which is used to color features with missing data.
