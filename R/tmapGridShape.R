...
tmapGridOverlay = function(bbx, facet_row, facet_col, facet_page, o) {
	...
	frame = if (!(is.na(frame.col))) {
		...
		if (o$earth.boundary) {
			...
			sf::st_as_grob(eotw, gp = grid::gpar(fill = NA, col = frame.col, lwd = frame.lwd), name = "outer_world", vp = grid::viewport(xscale = fbbx[c(1,3)], yscale = fbbx[c(2,4)]))
		} else {
