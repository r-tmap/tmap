...
map2divscaleID <- function(dvalues, n=101, contrast=1, rounding = TRUE, are_breaks = TRUE) {
    nv <- length(dvalues)
    ...
    if (lw==-Inf) lw <- dvalues[2]
    if (hg==Inf) hg <- dvalues[nv-1]
    mx <- max(abs(c(lw, hg)))


    is.div <- any(dvalues<0) && any(dvalues>0)

    cat0 = (are_breaks != any(dvalues==0))
    ...
    if (is.div && !cat0) {
        ...
        npos <- sum(dvalues>=0) - !is.div
        nneg <- sum(dvalues<=0) - !is.div
        step <- 0
    }
...
# @return vector of index numbers
map2seqscaleID <- function(dvalues, n=101, contrast=1, breaks.specified=TRUE, impute=TRUE, show.warnings = TRUE, are_breaks = TRUE) {
	#if (are_breaks_diverging(dvalues) && breaks.specified && show.warnings) warning("Breaks contains positive and negative values. Better is to use diverging scale instead, or set auto.palette.mapping to FALSE.", call. = FALSE)
	m <- (n*2)-1
	mh <- ((m-1)/2)+1
	ids <- map2divscaleID(dvalues, n=m, contrast=contrast, are_breaks = are_breaks)

	ids <- if (any(dvalues>0)) {
		ids - mh + 1
	} else {
		(mh+1) - ids
	}
