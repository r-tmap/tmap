...
leg_landscape = list(
	...
	fun_width = function(leg, o) {
		...
		w = if (leg$landscape.setup$item.width == -1 || leg$landscape.setup$space == -1) {
			Inf
		} else {
			nlines = sum(leg$lines)
			tW = ifelse(leg$title == "", 0, o$lin * o$legend.title.size * strwidth(leg$title, units = "inch"))
			iW = o$lin * (nlines + 0.8) * o$legend.text.size * (leg$landscape.setup$item.width + leg$landscape.setup$space)
			
			max(c(tW, iW)) + (o$lin * o$legend.text.size * leg$landscape.setup$margin * 2)
		}
	...
	fun_plot = function(leg, o) {
		...
		vp = grid::viewport(layout = grid::grid.layout(ncol = nlev + 2, nrow = 7, 
													   widths = grid::unit(
													   	c(lH * o$legend.text.size * leg$landscape.setup$margin,
													   	  rep(1/nlev, nlev),
													   	  lH * o$legend.text.size * leg$landscape.setup$margin, 1), units = c("inches", rep("null", nlev), "inches")),
													   heights = grid::unit(
		...
		if (leg$type == "gradient") {
			...
			if (leg$na.show) {
				gparleg$fill = gp$fill[ifelse(length(gp$fill), nlev, 1)]
				gparleg$fill_alpha = gp$fill[ifelse(length(gp$fill_alpha), nlev, 1)]
			...
			if (diffAlpha) {
				...
				if (leg$landscape.setup$item.width == -1) {
					grItems = mapply(function(i, gpari) gridCell(4, i+1, grid::rectGrob(width = grid::unit(1,"npc") - grid::unit(lH * o$legend.text.size * (leg$landscape.setup$space), "inches"), height = grid::unit(lH* o$legend.text.size, "inches"), gp = gpari)), 1:nlev, gpars, SIMPLIFY = FALSE)
				} else {
					grItems = mapply(function(i, gpari) gridCell(4, i+1, grid::rectGrob(width = grid::unit(lH * o$legend.text.size * leg$landscape.setup$item.width, "inches"), height = grid::unit(lH* o$legend.text.size, "inches"), gp = gpari)), 1:nlev, gpars, SIMPLIFY = FALSE)
				}
