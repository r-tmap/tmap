...
#' @param unit unit specification. Either \code{"metric"} (default) or \code{"imperial"}. Used to specify the scale bar (see \code{\link{tm_scale_bar}}) and to calculate densities for choropleths (see argument \code{convert2density} in \code{\link{tm_fill}}). \code{unit} can also be defined as "km", "m", "mi", or "ft" for respectively squared kilometers, meters, miles and feet. See also the underlying function\code{\link[tmaptools:approx_areas]{approx_areas}}, of which the arguments \code{coords.unit}, \code{unit.size} and \code{total.area} can be passed on.
