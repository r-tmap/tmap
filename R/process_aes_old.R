process_aes_old <- function(data, g, gt, gby, z, interactive, aes) {
	## general variables
	npol <- nrow(data)
	by <- data$GROUP_BY
	...
	x <- lapply(aes, function(a) g[[a]])
	
	## general color aesthetic, color NA, alpha checks / defaults
	if (length(x[[1]])==1 && is.na(x[[1]])[1]) x[[1]] <- gt$aes.colors["lines"]
	...
	if (nlevels(by)>1 && any(sapply(x, length) > 1)) {
	...
	x <- lapply(x, function(xi) {
		if (length(xi) < nx) rep(xi, length.out=nx) else xi
	})
	
	xvary <- lapply(x, function(xi) {
		all(xi %in% shpcols) && !is.null(xi)
	})
	...
	dts <- mapply(function(ai, xi, vi) {
		fsname <- paste0("free.scales.", names(ai))
		if (ai == "col") {
			if (vi) {
				is.colors <- FALSE
			} else {
				is.colors <- all(valid_colors(xi))
				if (!is.colors) stop("Invalid line colors", call. = FALSE)
				xi <- do.call("process_color", c(list(col=col2hex(xi), alpha=g$alpha), gt$pc))
				for (i in 1:nx) data[[paste("COLOR", i, sep="_")]] <- xi[i]
				xi <- paste("COLOR", 1:nx, sep="_")
			}
			
			
			dt <- process_data(data[, xi, drop=FALSE], by=by, free.scales=gby[[fsname]], is.colors=is.colors)
			
			dcr <- process_dtcol(dt, sel, g, gt, nx, npol, reverse=g$legend.col.reverse)
			if (dcr$is.constant) xi <- rep(NA, nx)
			dcr$is.colors <- is.colors
			dcr
		} else if (ai == "lwd") {
			
			if (!vi) {
				if (!all(is.numeric(xi))) stop("Line widths are neither numeric nor valid variable name(s)", call. = FALSE)
				for (i in 1:nx) data[[paste("lwd", i, sep="_")]] <- xi[i]
				xi <- paste("lwd", 1:nx, sep="_")
				gby[[fsname]] <- FALSE
				split.by <- FALSE
			} else split.by <- TRUE
			
			dt <- process_data(data[, xi, drop=FALSE], by=by, free.scales=gby[[fsname]], is.colors=FALSE, split.by=split.by)
			
			dcr <- process_dtlwd(dt, sel, g, gt, nx, npol, reverse=g$legend.col.reverse)
	...
	sel <- if (is.list(dtlwd)) {
		...
		if (is.list(dtcol)) {
			...
			lapply(split(dtlwd, f = f), function(i) !is.na(i))
		} else !is.na(dtlwd)
	} 
	...
	line.col.legend.title <- rep(if (is.ena(g$title.col)[1]) xcol else g$title.col, length.out = nx)
	line.lwd.legend.title <- rep(if (is.ena(g$title.lwd)[1]) xlwd else g$title.lwd, length.out = nx)
	...
	if (any(!lwd.legend.show)) line.lwd.legend.title[!lwd.legend.show]
