...
#' @param device graphic device to use. Either a device function (e.g., [grDevices::png()] or [grDevices::cairo_pdf()]) or a text indicating selected graphic device: "pdf", "eps", "svg", "wmf" (Windows only), "png", "jpg", "bmp", "tiff". If NULL, the graphic device is guessed based on the `filename` argument.
...
#' @param insets_vp [grid::viewport()] of an inset map, or a list of [grid::viewport()]s of multiple inset maps. The number of viewports should be equal to the number of tmap objects specified with `insets_tm`.
#' @param add.titles add titles to leaflet object
#' @param in.iframe should an interactive map be saved as an iframe? If so, two HTML files will be saved; one small parent HTML file with the iframe container, and one large child HTML file with the actual widget. See [widgetframe::saveWidgetframe()] for details. By default `FALSE` which means that one large HTML file is saved (see [htmlwidgets::saveWidget()]).
...
#' @param ... arguments passed on to device functions or to [htmlwidgets::saveWidget()] or [widgetframe::saveWidgetframe()]
