...
tmapAesAlpha = function(x1, setup, legend, opt) {
	if (!is.numeric(x1) && !is.factor(x1)) x1 = factor(x1)
	...
	if (is.na(setup$range[1])) setup$range = getTmapOption(tmapOption("aes.var", setup$aes), opt)
	if (is.na(setup$alphaNA)) setup$alphaNA = getTmapOption(tmapOption("aes.na", setup$aes), opt)
	if (is.na(setup$alphaNULL)) setup$alphaNULL = getTmapOption(tmapOption("aes.null", setup$aes), opt)
