	...
	structure(within(tml, {
		aes.mapping = if (!length(aes.mapping)) {
			...
			lapply(aes.mapping, function(a) {
				...
				dim = if (is.null(name)) 0L else if (name %in% dtcols) length(name) else 0L
				setup = get(paste(a, "setup", sep = "."))
				structure(list(name = name, dim = dim, setup = setup), class = c("tmapAes", "list"))
			})
		}
