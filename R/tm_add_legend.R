...
#'
#' @param ... map variables and arguments passed on to `tm_legend()`.
#'   By default, the argument `type` is set to `"symbols"`, which means that the
#'   supported map variables are: `"fill"`, `"col"`, `"shape"`, `"size"`,
...
#' @param type the layer type from which the map variables (see `...`) are taken.
