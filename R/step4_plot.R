...
process_components = function(cdt, o) {
	...
	cdt$cell.h = sapply(cdt$comp, function(l) {x = l$position$cell.h; if (is.null(x)) NA_character_ else x})
	cdt$cell.v = sapply(cdt$comp, function(l) {x = l$position$cell.v; if (is.null(x)) NA_character_ else x})
	cdt$pos.h = sapply(cdt$comp, function(l) {x = l$position$pos.h; if (is.null(x)) NA_character_ else x})
	cdt$pos.v = sapply(cdt$comp, function(l) {x = l$position$pos.v; if (is.null(x)) NA_character_ else x})
...
step4_plot = function(tm, vp, return.asp, show, in.shiny, knit, knit_opts, args) {
			...
			for (qi in which(q$new)) {
				...
				if (gid > 0) {
					...
					if (nrow(mdt) != 0) {
						...
						do.call(FUN, c(list(a = a, shpTM = shpTM, dt = mdt, pdt = bl$popup.data, popup.format = bl$popup.format, ptdt = bl$popup.title.data, popup.layout = bl$popup.layout, hdt = bl$hover.data, idt = bl$id.data, gp = gp, bbx = bbx, facet_col = d$col[i], facet_row = d$row[i], facet_page = d$page[i], id = id, pane = pane, group = group, glid = bl$glid, o = o)))
					}
	...
	if (nrow(cdt) > 0L) for (k in seq_len(o$npages)) {
		...
		# find group ids
		klegs$id = paste(klegs$class, klegs$cell.h, klegs$cell.v, klegs$pos.h, klegs$pos.v,
						 sapply(klegs$comp, function(comp) paste(comp$position$just.h, comp$position$just.v, sep = "_")), sep = "_")
		# just.id = NULL
