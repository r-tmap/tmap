...
#' @param height height of the arranged output. Only used in the interactive
#'   modes (`"view"`, `"maplibre"`, `"mapbox"`). A bare number is interpreted as
#'   pixels; a string is used as-is (e.g. `"80vh"`). The default `NULL` fills the
...
#' @export
tmap_arrange = function(..., ncol = NA, nrow = NA, widths = NA, heights = NA, sync = FALSE, asp = 0, outer.margins = .02, height = NULL) {
	tms = list(...)
	...
	opts = list(ncol=ncol, nrow=nrow, widths=widths, heights=heights, sync=sync, asp=asp, outer.margins=outer.margins, height=height)
	attr(tms, "opts") = opts
