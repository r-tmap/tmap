...
#'
#' Specify a constant value for a map variable (e.g. a fixed fill color or symbol size), bypassing data-driven scaling.
#'
#' @export
tm_const = function() {
...
#' and the map variable. The tmap option `scales.var` contains information
...
#' The function `tm_scale_asis()` is used to take data values as they are and use them as such for the map variable.
#'
...
tm_scale_ordinal = function(n.max = 30,
							values = NA, # e.g. palette, shape numbers, set of sizes (if two values are specified they are interpreted as a range)
							values.repeat = FALSE,
							values.range  = 1,
							values.scale = NA,
...
#' For colors (the map variable `fill` or `col`), different default color
...
#' @param values (generic scale argument) The visual values. For colors (e.g. `fill` or `col` for `tm_polygons()`) this is a palette name from the `cols4all` package (see [cols4all::c4a()]) or vector of colors, for size (e.g. `size` for `tm_symbols()`) these are a set of sizes (if two values are specified they are interpreted as a range), for symbol shapes (e.g. `shape` for [tm_symbols()]) these are a set of symbols, etc. The tmap option `values.var` contains the default values per map variable and in some cases also per data type.
...
#' @param values.scale (generic scale argument) Scaling of the values. Only useful for size-related map variables, such as `size` of [tm_symbols()] and `lwd` of [tm_lines()].
#' @param value.na (generic scale argument) Value used for missing values. See tmap option `"value.na"` for defaults per map variable.
#' @param value.null (generic scale argument) Value used for NULL values. See tmap option `"value.null"` for defaults per map variable. Null data values occur when out-of-scope features are shown (e.g. for a map of Europe showing a data variable per country, the null values are applied to countries outside Europe).
#' @param value.neutral (generic scale argument) Value that can be considered neutral. This is used for legends of other map variables of the same map layer. E.g. when both `fill` and `size` are used for [tm_symbols()] (using filled circles), the size legend items are filled with the `value.neutral` color from the `fill.scale` scale, and fill legend items are bubbles of size `value.neutral` from the `size.scale` scale.
...
#' @param label.style Either `"discrete"` or `"continuous"`. If `discrete` (default) intervals will be labeled. If `continuous`, the legend will be similar to a continuous scale legend (but with discrete colors), where the breaks are labeled.
...
#' @param limits Limits of the data values that are mapped to the continuous scale. When `NA`, the range of data values is taken. When only one value is provided, the range of data values with this provided value is taken. The default depends on the map variable: it is 0 for all map variables other than color when `tm_scale_continuous` is used. For the transformation scale functions, it is `NA`.
