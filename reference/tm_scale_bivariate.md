...
- scale1, scale2:
  ...
  two `tm_scale` objects. Currently, all `tm_scale_*()` functions are
  supported except
...
- values.range:
  ...
  is encoded as `c(0, 1)`. For instance, when a grey scale is used for
  color (from black to white), `c(0,1)` means that all colors are used,
  `0.25, 0.75` means that only colors from dark grey to light grey are
