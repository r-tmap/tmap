...
- n.max:
  ...
  Maximum number of categories (factor levels). In case there are more,
  they are grouped into `n.max` groups.
...
- values.range:
  ...
  that only colors are used from black to middle gray (`"gray50"`). When
...
- levels.drop:
  ...
  Should unused levels be dropped (and therefore are not assigned to a
  visual value and shown in the legend)?
