...
mp = World2$geometry[[136]]

mp[] = lapply(mp, function(mpi) {
	lapply(mpi, function(mpii) {
		mpii[,1][mpii[,1] < 0] = mpii[,1][mpii[,1] < 0] + 360
		mpii
	})
})

World2$geometry[[136]] = mp

which(World2$iso_a3 == "FJI")
qtm(World2)

mp = World2$geometry[[54]]
...
World2$geometry[[54]] = mp

which(World2$iso_a3 == "ATA")

mp = World2$geometry[[7]]


mp[[8]] = lapply(mp[[8]], function(mpii) {
	mpii[,1][mpii[,1] < -160] = mpii[,1][mpii[,1] < -160] + 360
	mpii[,2][mpii[,2] < -84.8] = min(mpii[,2])
	mpii[,2][mpii[,2] < -84.8] = min(mpii[,2])
	mpii[!(mpii[,1] < -143.10775 & mpii[, 2] < -85), ]
})
World2$geometry[[7]] = mp
...
qtm(World2, projection = "+proj=robin")
...
W = giscoR::gisco_get_countries(resolution = "60", year = "2016") %>% select(NAME_ENGL, ISO3_CODE)
all(st_is_valid(W))
...
W_global = ne_countries(returnclass = "sf")
W_detail <- ne_countries(returnclass = "sf", scale = 10)


setdiff(W$ISO3_CODE, W_detail$adm0_a3)
...
WBareas <- read.csv(file.path(tmpdir, "API_AG.LND.TOTL.K2_DS2_en_csv_v2_2446063.csv"), skip=4, stringsAsFactors=FALSE)
