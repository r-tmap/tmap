...
W_detail_GallPeters$area <- as.numeric(st_area(W_detail_GallPeters))
W$area_from_shp <- W_detail_GallPeters$area[match(W$name_long, W_detail_GallPeters$name_long)] / 1e6
...
WBareas <- read.csv(file.path(tmpdir, "API_AG.LND.TOTL.K2_DS2_en_csv_v2.csv"), skip=4, stringsAsFactors=FALSE)
...
W2$area_diff <- ((W2$area_from_WB - W2$area_from_shp) / W2$area_from_shp) * 100
...
W2$pop_est_dens <- W2$pop_est / W2$area
...
qtm(W3, fill="HPI", fill.id = "name", fill.palette = "RdYlGn", fill.auto.palette.mapping = FALSE)
qtm(World, fill="HPI", fill.id = "name")
