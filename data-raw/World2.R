...
# STEP 1
W = ne_countries(scale = 110)
W = st_make_valid(W)

## STEP 2 Download detailed shape to extract area size (these areas will be used as fallback source. The main source of area sizes is the World Bank (see STEP 5 below)
W_detail = ne_countries(scale = 10)
W_detail = st_make_valid(W_detail)
a = st_area(W_detail)

data(World)

cc = read.csv("sandbox/country code.csv")

W = read.csv("sandbox/world-data-2023.csv")
W = W[!is.na(W$Latitude),]

W$iso_a3 = cc$Alpha.3.code[match(W$Abbreviation, cc$Alpha.2.code)]

library(dplyr)

World = left_join(World, W, by = "iso_a3")

World$Unemployment.rate = as.numeric(substr(World$Unemployment.rate, 1, nchar(World$Unemployment.rate) - 1))
World$Co2.Emissions = as.numeric(stringr::str_replace(World$Co2.Emissions, ",", ""))

tm_shape(World) +
	tm_polygons("Unemployment.rate")

tm_shape(World) +
	tm_bubbles(size = "Co2.Emissions")
