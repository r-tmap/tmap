...
```{r}
leg = tm_legend_portrait(frame = TRUE, text.size = 1.5, text.fontface = "italic", text.fontfamily = "DejaVu Serif")
scl = tm_scale_intervals(label.na = "Abcdef")
tm_shape(World) +
	tm_polygons("HPI", fill.legend = leg, fill.scale = scl)
```


```{r}
leg = tm_legend_portrait(item.na.padding = 1)
tm_shape(Africa) +
	tm_symbols(size = "HPI", size.legend = leg)
```
...
leg = tm_legend_portrait(bg.color = "skyblue", frame.lwd = 4)
tm_shape(World) +
	tm_polygons("HPI", col = "economy", fill.legend = leg)

```


```{r}
leg = tm_legend_portrait()
tm_shape(World) +
	tm_symbols("HPI", size.scale = ,fill.legend = leg)
```


```{r}
leg = tm_legend_landscape()
tm_shape(World) +
	tm_polygons("HPI", fill.legend = leg)
```


```{r}
tm_shape(World) +
	tm_symbols(size = "HPI", size.legend = tm_legend_landscape())
```
...
tm_shape(World) +
	tm_polygons("HPI", fill.legend = leg, fill.scale = scl, col = "economy", col.legend = tm_legend_portrait(position = tm_lp_out(h="right", v="center")))
```
