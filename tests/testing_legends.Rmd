...
```{r}
tmap_options(legend.design = "portrait")
tmap_design_mode()
tm_shape(World) +
	tm_polygons("HPI", col = "economy")

tmap_options(legend.frame=TRUE,
			legend.settings = list(portrait = list(item.height = c(rect = 2, symbols = 1, gradient = 1.8),
											   item.width = c(rect = 2, symbols = 1, gradient = 1),
...
tm_shape(World) +
	tm_polygons("HPI", col = "economy", fill.legend = tm_legend(space = 0, space.na = 4))
tm_shape(World) +
	tm_symbols(size = "HPI", fill = "economy")
