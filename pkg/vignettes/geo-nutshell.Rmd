...
### Plotting with geo elements<a name="plot"></a>
...
|---	|---	|
|[`geo_fill`](../html/geo_fill.html)   	| col<sup>1</sup>  	|
|[`geo_bubbles`](../html/geo_bubbles.html)   	| size<sup>1</sup>, col<sup>1</sup>  	|
|[`geo_lines`](../html/geo_lines.html)		| col<sup>1</sup>, lwd<sup>1</sup>		|
|[`geo_text`](../html/geo_text.html)   	| text<sup>2</sup>, cex<sup>3</sup>  	|
|[`geo_borders`](../html/geo_borders.html)   	| col<sup>4</sup>, lwd<sup>4</sup>  	|
...
2. The text attribute of [`geo_text`](../html/geo_text.html) should be the name of a data variable.
3. The cex attribute of [`geo_text`](../html/geo_text.html) can be used for a constant value and a statistical variable (like number 2), but no legend is generated.
4. Only constant values can be assigned to these arguments. For instance `geo_borders(col="gray50", lwd=2)`.
...
* Plotting shape objects
    + With the [quick plotting](#qplot) function [`geo`](../html/geo.html), or the [main plotting method](#plot).
* Saving the output for publication and presentation
    ...
    geo_shape(Europe) +
    	...
    	geo_borders() +
    	geo_text("iso_a3", cex="pop_est") + 
    geo_theme("GDP per capita", scale=1)      
    dev.off()
    ```
    The `scale` argument in [`geo_theme`](../html/geo_theme.html) is very useful as overall scalar (similar to the `scale` argument in `ggplot2`'s `ggsave`).
    + Create animations (gif or mpeg) from a series of small multiples with [`animation_geo`](../html/animation_geo.html):
    ...
    ```
    Notice that, in order to create a series of plots where one map is shown at a time, both `nrow` and `ncol` are set to 1 in [`geo_facets`](../html/geo_facets.html).
