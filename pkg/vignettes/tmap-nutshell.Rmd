...
qtm(Europe, fill="gdp_cap_est", text="iso_a3", text.size="AREA", root=5, fill.title="GDP per capita", 
	fill.textNA="Non-European countries", format="Europe")
```
...
|[`tm_bubbles`](../html/tm_dots.html)   	| Draws dots | col  	|
|[`tm_lines`](../html/tm_lines.html)		| Draws polylines | col, lwd	|
|[`tm_lines`](../html/tm_raster.html)		| Draws a raster | col	|
...
tm_shape(metro) +
    tm_text("name", size="pop2010", scale=1, root=4, size.lowerbound = .6, 
        bg.color="white", bg.alpha = .25, auto.placement = TRUE) + 
...
tm_shape(Europe) +
	tm_text("iso_a3", size="area", col = "gray35", scale=1.5, root=8, size.lowerbound = .40, 
        fontface="bold", case=NA) + 
tm_format_Europe(title="Map of Europe", bg.color="lightblue2")
```
...
* The shape objects can have different projections, and can also cover different areas (bounding boxes). Both the projection and the covered area are by default taken from shape object defined in the first [`tm_shape`](../html/tm_shape.html). Notice that the rivers shape object also contains rivers outside Europe: see `tm_shape(rivers) + tm_lines("dodgerblue3")`. Use [`tm_shape`](../html/tm_shape.html)'s `is.master` argument to take the projection and covered area from other shape objects.
...
### Designing
The layout of the thematic map can be changed with [`tm_layout`](../html/tm_layout.html). In the next example we use two wrapper functions, one for the overall format of world maps, and one for the legend.
