...
We refer to **shape objects** as objects from the class `Spatial` from the `sp` package, or `Raster` from the `raster` package. The six supported subclasses are:
...
tm_shape(Europe) +
	tm_fill("pop_est_dens", style="kmeans", textNA="Non-European countries", title="Country population density (per km2)") +
	tm_borders() +
tm_shape(rivers) +
...
tm_shape(metro) +
	...
	tm_bubbles("X2010", "red", border.col = "black", border.lwd=1, size.lim = c(0, 11e6), sizes.legend = seq(2e6,10e6, by=2e6), title.size="City Population") +
tm_shape(Europe) +
