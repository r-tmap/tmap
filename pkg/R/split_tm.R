...
get_i <- function(x, xname, i, n, oid) {
	...
	if (is.null(x)) {
		...
		if (xname %in% c("symbol.size.legend.palette", "symbol.max.size", "line.lwd.legend.palette", "line.legend.lwd", "text.max.size", "text.size.legend.palette") || substr(xname, ncx-11, ncx)=="legend.title") {
			if (length(x)>=i) x[i] else x[1]
		} else x
	}
}
