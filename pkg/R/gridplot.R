	...
	treeMlts <- lapply(1:np, function(k) {
		...
		if (panel.mode=="both") {
			...
			colPanels <- lapply((1:mfcol), function(i) {
				cellplot(gmeta$colpanelrow, gmeta$rowrange[i], e=gList(rectGrob(gp=gpar(fill=gmeta$panel.label.bg.color)),
		...
		if (!is.null(gp_leg)) {
			#legPanel <- gList(cellplot(gmeta$legy, gmeta$legx, e=gList(rectGrob(gp=gpar(fill="gold")))))
		...
		tree <- gTree(children=do.call("gList", c(list(grobBG, grobFacetBG), treeMults, rowPanels, colPanels, legPanel)), vp=vpGrid)
