...
#' @param key.data variable name of \code{data} to be matched with \code{key.shp}
#' @param key.shp variable name of \code{shp} map data to be matched with \code{key.data}
...
#' @export
appendData <- function(shp, data, key.data = NULL, key.shp = NULL, ignore.duplicates=FALSE) {
	...
	if (setequal(ids.data, ids.shp)) {
		...
		if (!all(ids.shp %in% ids.data)) {
			...
			warning(paste("Under coverage. No data for:", paste(notMatched.shp, collapse=", ")))
		}				
		if (!all(ids.data %in% ids.shp)) {
			notMatched.data <- setdiff(ids.data, ids.shp)
			warning(paste("Over coverage. Unmatched data records:", paste(notMatched.data, collapse=", ")))
		}				
	}
