grid.shape2 <- function(shp, gp=gpar(), bg.col=NA) {
	bb <- bbox(shp)
...
grid.shape <- function(shp, gp=gpar(), bg.col=NA) {
	...
	idMain <- co1[,5]
	idNames <- paste("tm_polygons", idMain, sep="_")
	np <- length(shp@polygons)
	
	do.call("gList", lapply(1:np, function(i) {
		sel <- idMain==i
		polygonGrob(co1[sel,1], co1[sel,2],	id=id[sel], gp=gp2, name = paste("tm_polygons", i, sep="_"))
	}))
}

grid.shape3 <- function(shp, gp=gpar(), bg.col=NA) {
	# TODO substract holes
	bb <- bbox(shp)
	co1 <- do.call("rbind", mapply(function(p, id1) {
		co2 <- mapply(function(pp, id2) {
