...
#' @export
#' @import raster
sample_dots <- function(shp, vars=NULL, convert2density=TRUE, nrow=NA, ncol=NA, N=250000, npop=NA, n=10000, w=NA, shp.id=NULL, var.name="class", var.labels=vars, unit="km", unit.size=1000, ...) {
	args <- list(...)
	...
	if (convert2density) {
		...
		data <- do.call("calc_densities", args = c(list(shp=shp, var=vars, unit=unit, unit.size=unit.size), dens_args))
	} else {
	...
	r2 <- rasterize(shp, r, field=data$ID__POLY) #raster with buurt id's
	...
	distr <- sapply(g@data[, vars], function(x) {
	...
	distr2 <- t(apply(distr, MARGIN = 1, function(x){
		pos <- sum(x!=0)
		if (pos > 1) {
	...
	samples <- mapply(function(x, size) {
		s <- integer()
		while(length(s)<size) {
			snew <- sample.int(n=N, size= size, replace=TRUE, prob=x)
			s <- unique(c(s, snew))
		}
		(1:N2)[match(s[1:size], notNA)]
	...
	if (!missing(shp.id)) {
		p2[[shp.id]] <- shp[[shp.id]][p2$ID__POLY] 
	} else {
