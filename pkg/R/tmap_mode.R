...
#'    	\item{\code{"view"}}{Thematic maps are viewed interactively in the web browser or RStudio's Viewer pane. Maps are fully interactive with tiles from OpenStreetMap or other map providers. However, there are a couple of contraints in comparison to \code{"plot"}:
#'    	\itemize{
...
#'    	See \code{\link{tm_view}} for options related to the \code{"view"} mode. This mode makes use of \code{\link[leaflet:leaflet]{leaflet}}. A htmlwidget is created that is based on the Javascript library leaflet. With RMarkdown, it is possible to publish it to an HTML page.
#'    	}}
